...
## Grading
The final grade for this project will be calculated as
```
Grade = x*shell_score/shell_max + y*net_score/net_max + z*xv6_score/xv6_max
```

Where `x=0.4`, `y=0.3`, and `z=0.3`.
...
## Part A - Basic System Call: getreadcount \[40\]
...
### Requirements
#### A.1: System Call Implementation \[30\]
...
We will modify the xv6 scheduler from a strict round-robin to a first come first serve (FCFS) scheduler. This will involve using the creation time entry in the process control block that was added in part A. We will modify the scheduler function (kernel/proc.c). Scheduler will first have to find a RUNNABLE process with the earliest creation time. The process with the earliest arrival time is the process with the highest priority and therefor the process that is selected for execution. Only when the currently RUNNING process terminates is another process selected to RUN. It is suggested that you comment out the original round-robin scheduler code before adding your new version of the scheduler.
